...
class AuthRemoteRepository {
  ...
  Future<Either<Failure, Unit>> requestOtp(String phoneNumber) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return left(
          Failure('Erreur lors de la demande OTP: ${response.statusMessage}'),
        );
      }
  ...
  Future<Either<Failure, AuthUser>> getUser(String token) async {
    try {
      ...
      return left(
        Failure('Erreur lors de la récupération des informations: $e'),
      );
    }
  }
  ...
  Future<Either<Failure, Unit>> updateProfile({
    ...
    try {
      final response = await _dio.post(
        ...
        data: {"phone": phone, "firstname": firstName, "lastname": lastName},
      );
      if (response.statusCode == 200 || response.statusCode == 201) {
        return right(unit);
      } else {
